  ...
  const register = async (req, res, next) => {
   ...
   const login = async (req, res, next) => {
    try {
      ...
      // Fetch the user by email and password (this function returns a Mongoose document)
      ...
      await fetchedUser.save(); // Save the updated lastLogin field to the database
  
      // Exclude the password from the returned user
      const { password: _, ...userWithoutPassword } = fetchedUser.toObject(); // Convert to plain object
  
      // Generate authentication tokens
      const tokens = await generateAuthTokens(userWithoutPassword);
  
      // Send response with user (excluding password) and tokens
   ...
   const refreshToken = async (req, res, next) => {
    try {
      ...
      const {password:_, ...user} = await verifyUserFromRefreshTokenPayload(refreshTokenPayload);
...
export default { 
  login, logout, refreshToken,  resetPassword, register, me
}